...
def get_columns():
    return [
        ...
        {
            "label": _("Reason for Exit"),
            "fieldname": "custom_reason_of_eos",
            "fieldtype": "Data",
        },
        {
            "label": _("EOS Years"),
            "fieldname": "custom_eos_years",
            "fieldtype": "Float",
            "width": 100,
        },
        # {
...
def get_data(filters):
    ...
    query = (
        ...
        .select(
            ...
            employee.custom_reason_of_eos.as_("custom_reason_of_eos"),
            employee.custom_eos_years.as_("custom_eos_years"),
            employee.custom_end_of_service_amount.as_("custom_end_of_service_amount"),
            employee.custom_net_end_of_service_amount.as_(
        ...
        .where(
            ...
            & (
                (interview.name.isnull())
                | ((interview.name.isnotnull()) & (interview.docstatus != 2))
            )
...
def get_conditions(filters, query, employee, interview, fnf):
    ...
    # filter joining date
    if filters.get("joining_from_date") and filters.get("joining_to_date"):
        query = query.where(
            employee.date_of_joining[
                getdate(filters.get("joining_from_date")) : getdate(
                    filters.get("joining_to_date")
                )
            ]
        )
    elif filters.get("joining_from_date"):
        query = query.where(
            employee.date_of_joining >= filters.get("joining_from_date")
        )
    elif filters.get("joining_to_date"):
        query = query.where(employee.date_of_joining <= filters.get("joining_to_date"))

    if filters.get("company"):
        query = query.where(employee.company == filters.get("company"))
    if filters.get("custom_reason_of_eos"):
        query = query.where(
            employee.custom_reason_of_eos == filters.get("custom_reason_of_eos")
        )
    ...
    if filters.get("questionnaire_pending"):
        query = query.where(
            (interview.reference_document_name == "")
            | (interview.reference_document_name.isnull())
        )
...
def get_report_summary(data):
    ...
    fnf_pending = len(
        [entry.name for entry in data if not entry.full_and_final_statement]
    )
    questionnaires_pending = len(
        [entry.name for entry in data if not entry.questionnaire]
    )
